...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        # --- Dual Decoder Projections ---
        
        # 1. Tail Selection Decoder (Modified for Dynamic Context)
        # Input: Fixed Context (Global) + Last Head Embedding -> Output: Query for Tail
        self.project_step_context_tail = nn.Linear(2 * embedding_dim, embedding_dim, bias=False)
        self.project_out_tail = nn.Linear(embedding_dim, embedding_dim, bias=False)
        
        # Placeholder for the first step where there is no "previous head"
    ...
    def _inner(self, input, embeddings):
        ...
        while not state.all_finished():
            ...
            # --- Step 1: Select Tail (With Cross-Step Communication) ---
            
            # Context: Global Graph Context + Previous Head Embedding
            # fixed.context_node_projected: (batch, 1, embed)
            # last_head_embedding: (batch, 1, embed)
            tail_context_input = torch.cat((fixed.context_node_projected, last_head_embedding), dim=-1) # (batch, 1, 2*embed)
            ...
            # --- Update State & Dynamic Context ---
            
            state = state.update(selected_tail, selected_head)

            # 更新 last_head_embedding 供下一步的 Tail 使用
            # 提取当前选中的 Head 的 embedding
            last_head_embedding = torch.gather(
                embeddings, 
                1, 
                selected_head[:, None, None].expand(batch_size, 1, self.embedding_dim)
            )
