...
import numpy as np
import torch
from secondary_component import Model
from component_adapter import teacher_state, State
...
def prepare(out,a):
    ...
    config=dict(protocol=Path(__file__).with_name('SECONDARY_PROTOCOL.md').read_text(),
                ...
                final_test_seeds=[73019211,73019212,73019213,73019214],steps=a.steps,batch=a.batch,host=a.host)
...
def smoke(out,a):
    ...
    for mode in ['route','random']:
        for depth in [0,7,18]:
            ...
            assert all(p.grad is None for p in m.host.parameters() if not p.requires_grad)
    ...
    opt=torch.optim.Adam([{'params':m.residual.parameters(),'lr':3e-4}, {'params':[p for p in m.host.parameters() if p.requires_grad],'lr':1e-5}])
    ...
    for seed in a.seeds:
        for aware in [False,True]:
            ...
            save_json(folder/'training_manifest.json',dict(host=a.host,
                sources={p.name:sha(p) for p in Path(__file__).parent.glob('*.py')},
                protocol_sha=sha(Path(__file__).with_name('SECONDARY_PROTOCOL.md')),data_sha=sha(out/'data.pt')))
            ...
            for step in range(a.steps+1):
                if step:
                    ...
                    loss.backward(); grad=torch.nn.utils.clip_grad_norm_(m.trainable_parameters(),1.)
                ...
                if step%1000==0 or step==a.steps:
                    ...
                    if normalized<best:
                        ...
                        torch.save(dict(**m.capture(),step=step,seed=seed,aware=aware,mode=mode),folder/'best.pt')
            ...
            torch.save(dict(**m.capture(),step=a.steps),folder/'final.pt')
            save_json(folder/'training_summary.json',dict(seed=seed,mode=mode,aware=aware,best_step=best_step,
                      ...
                      parameters=sum(p.numel() for p in m.trainable_parameters()),decoder_parameter_names=m.decoder_names))
...
def finalize(out,a):
    ...
    for name,seed,count,n,clustered in [('uniform100',73019211,256,100,False),('cluster100',73019212,128,100,True),
                                      ('uniform200',73019213,128,200,False),('cluster200',73019214,128,200,True)]:
    ...
    for folder in folders:
        ck=torch.load(folder/'best.pt',weights_only=False); m.aware=ck['aware']; m.restore(ck)
