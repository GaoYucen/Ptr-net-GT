#!/usr/bin/env bash
# Single-seed, two-GPU TSP50 idea screen.
set -euo pipefail

out_root=${1:?usage: run_pair_validation_queue.sh OUTPUT_ROOT [SEED] [GPU_A] [GPU_B]}
seed=${2:-12031}
gpu_a=${3:-0}
gpu_b=${4:-1}
...
run_one() {
    ...
    test -e "$out/summary.json" && return 0
    test -e "$out" && {
      echo "incomplete output exists: $out" >&2
      return 1
    }
...
# Baseline first; the two hypotheses then occupy both cards concurrently.
run_one single_opt "$gpu_a"
run_one pair_add "$gpu_a" & pid_add=$!
run_one pair_interaction "$gpu_b" & pid_interaction=$!
wait "$pid_add"
wait "$pid_interaction"
