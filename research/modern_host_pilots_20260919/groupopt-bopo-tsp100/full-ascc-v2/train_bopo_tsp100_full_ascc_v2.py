...
CHECKPOINT = BOPO_TSP / "result/saved_n100/checkpoint-700.pt"
...
class FullASCCAdapter(nn.Module):
    def __init__(self, d: int = 128, heads: int = 8, qkv: int = 16, clip: float = 10.0):
        ...
        # Trainable endpoint residual on top of the frozen BOPO endpoint distribution.
        # Identity/zero initialization starts from the native BOPO head policy.
        self.head_tail = nn.Linear(d, d, bias=False)
        self.head_node = nn.Linear(d, d, bias=False)
        nn.init.eye_(self.head_tail.weight)
        nn.init.zeros_(self.head_node.weight)

    def head_log_probabilities(
        self,
        tail_e: torch.Tensor,
        head_e: torch.Tensor,
        frozen_log_p: torch.Tensor,
    ) -> torch.Tensor:
        q = self.head_tail(tail_e)
        k = self.head_node(head_e)
        residual = torch.matmul(q, k.transpose(1, 2)) / math.sqrt(self.d)
        mask = ~torch.isfinite(frozen_log_p)
        logits = frozen_log_p + residual
...
def rollout_full_ascc(
    ...
    head_log_likelihood = torch.zeros(r, device=device)
    entropy_sum = torch.zeros(r, device=device)

    for edge in range(n):
        tails, heads, frozen_head_log_p, _ = bopo_all_source_head_proposal(
            bopo, enc, coords, succ, pred, comp, edge
        )
        tail_e = gather_nodes(enc, tails)
        head_e = gather_nodes(enc, heads)
        packed_head_log_p = adapter.head_log_probabilities(
            tail_e, head_e, frozen_head_log_p
        )
        tail_xy_all = gather_nodes(coords, tails)
        head_xy_all = gather_nodes(coords, heads)
        distances_all = (
            tail_xy_all[:, :, None, :] - head_xy_all[:, None, :, :]
        ).norm(p=2, dim=-1)
        finite_head = torch.isfinite(packed_head_log_p)
        probs_head = torch.where(
            finite_head, packed_head_log_p.exp(), torch.zeros_like(packed_head_log_p)
        )
        safe_head = torch.where(
            finite_head, packed_head_log_p, torch.zeros_like(packed_head_log_p)
        )
        packed_summary = torch.stack(
            (
                -(probs_head * safe_head).sum(-1) / math.log(max(n, 2)),
                (probs_head * distances_all).sum(-1) / math.sqrt(2.0),
                probs_head.amax(-1),
            ),
            dim=-1,
        )
        summary_full = torch.zeros((r, n, 3), device=device, dtype=enc.dtype)
        summary_full.scatter_(
            1,
            tails[:, :, None].expand(r, tails.size(1), 3),
            packed_summary.detach(),
        )
        ...
        if sample_source:
            selected_head_pos = torch.multinomial(
                selected_head_log_p.exp(), 1, generator=action_generator
            ).squeeze(1)
        else:
            selected_head_pos = selected_head_log_p.argmax(-1)
        selected_head = heads[batch, selected_head_pos]
        selected_head_action_log_p = selected_head_log_p.gather(
        ...
        head_log_likelihood = head_log_likelihood + selected_head_action_log_p
    ...
    return total_cost, source_log_likelihood + head_log_likelihood, entropy_sum / float(n)
...
def evaluate_official(
    ...
    for step in range(0, 1000, 100):
        problems, _ = torch.load(
            BOPO_TSP / f"data/tsp_n100_{step}.pkl",
            map_location="cuda",
            weights_only=False,
        )
        chunks = []
        for a in range(0, problems.size(0), chunk_augmented):
            ...
            costs, _, _ = rollout_full_ascc(adapter, bopo, c, enc, 100, False)
            chunks.append(costs.reshape(c.size(0), 100).cpu())
        batch_costs = torch.cat(chunks, dim=0).reshape(8, 100, 100)
...
def main():
    ...
    validation = torch.rand((args.validation_size, 100, 2), generator=val_g).to(device)
    ...
    for step in range(1, args.steps + 1):
        adapter.train()
        coords = torch.rand((args.batch_size, 100, 2), generator=data_g, device=device)
        enc = encode_frozen(bopo, coords)
    ...
    summary = {
        ...
        "mechanism": "frozen_bopo_plus_trainable_tail_plus_trainable_head_residual_with_stopped_head_summary",
        "host": "BOPO-2025",
        "host_upstream_commit": "2739fbfe39a478755173c892b80c4b06f9f59b05",
        "host_checkpoint": "checkpoint-700.pt",
        ...
        "native_bopo_endpoint_policy_frozen": True,
        "ascc_head_adapter_trainable": True,
        "encoder_frozen": True,
    }

    if not args.skip_final:
        raw, full_best, eval_seconds = evaluate_official(adapter, bopo, args.final_chunk)
        control = json.load(open("/workspace/groupopt-bopo-tsp100/baseline_summary.json"))
        full_best_d = full_best.double()
        control_mean = float(control["aug_mean"])
        full_mean = float(full_best_d.mean())
        absolute_mean_improvement = control_mean - full_mean
        ...
        summary.update({
            ...
            "matched_control_gap_percent": float(control["aug_gap_percent"]),
            "full_ascc_mean": full_mean,
            "absolute_mean_improvement": absolute_mean_improvement,
            "relative_improvement_percent": absolute_mean_improvement / control_mean * 100.0,
