...
class AttentionModel(nn.Module):
    ...
    def _decode_native_conditional_free(
        ...
        *,
        ...
        """Exact active-candidate Forest decoding.

        efficiency-v2 keeps the same legal policy distribution but removes inactive
        tails/heads before attention. At a state with ``r`` open path components the
        proposal is evaluated on ``r x r`` candidates rather than ``n x n``. The
        all-tail proposal remains stop-gradient; only the selected tail's head policy
        is re-scored with autograd during training.
        """
        process = self.construction_process
        state = process.initial_state(coordinates)
        graph_context = self.project_graph(graph_embedding)
        ...
        node_index = torch.arange(state.n, device=coordinates.device).unsqueeze(0).expand(
            state.batch_size, state.n
        )

        def active_indices(mask: Tensor, count: int) -> Tensor:
            # Preserve ascending global-node order so the compact softmax has the same
            # ordering as the dense implementation after masked entries are removed.
            sentinel = torch.full_like(node_index, state.n)
            ordered = torch.where(~mask, node_index, sentinel).sort(dim=1).values
            return ordered[:, :count]

        def gather_nodes(values: Tensor, indices: Tensor) -> Tensor:
            return values.gather(
                1, indices.unsqueeze(-1).expand(-1, -1, values.size(-1))
            )

        def gather_head_axis(values: Tensor, indices: Tensor) -> Tensor:
            # values: (B, H, N, D)
            return values.gather(
                2,
                indices[:, None, :, None].expand(
                    -1, values.size(1), -1, values.size(3)
                ),
            )
        ...
        while not process.is_terminal(state):
            open_count = state.n - state.edges_added
            tail_mask_full = process.base_mask(state)
            head_mask_full = state.predecessor >= 0
            active_tail = active_indices(tail_mask_full, open_count)
            active_head = active_indices(head_mask_full, open_count)

            active_tail_queries = gather_nodes(head_queries, active_tail)
            active_head_key = gather_head_axis(key, active_head)
            active_head_value = gather_head_axis(value, active_head)
            active_head_logit_key = gather_nodes(logit_key, active_head)

            if state.edges_added < state.n - 1:
                tail_component = state.component.gather(1, active_tail)
                head_component = state.component.gather(1, active_head)
                pair_mask = tail_component.unsqueeze(2) == head_component.unsqueeze(1)
            else:
            ...
            with torch.no_grad():
                all_head_logits = self._attention_logits(
                    active_tail_queries.detach(),
                    active_head_key.detach(),
                    active_head_value.detach(),
                    active_head_logit_key.detach(),
                    pair_mask,
                    self.project_head_glimpse,
                )
                ...
                active_distances = distances.gather(
                    1,
                    active_tail.unsqueeze(-1).expand(-1, -1, state.n),
                ).gather(
                    2,
                    active_head.unsqueeze(1).expand(-1, open_count, -1),
                )
                head_summary = native_head_summary(all_head_log_p, active_distances)

            if random_tail:
                full_tail_log_p = _random_forest_tail_log_probabilities(
                    tail_mask_full, node_embeddings.dtype, generator
                )
                selected_tail = full_tail_log_p.argmax(dim=1)
                selected_tail_pos = (
                    active_tail == selected_tail[:, None]
                ).to(torch.long).argmax(dim=1)
                tail_log_p = full_tail_log_p.gather(1, active_tail)
            elif fixed_tail:
                full_tail_log_p = _deterministic_forest_tail_log_probabilities(
                    state, tail_mask_full, node_embeddings.dtype
                )
                selected_tail = full_tail_log_p.argmax(dim=1)
                selected_tail_pos = (
                    active_tail == selected_tail[:, None]
                ...
                state_tail_nodes = gather_nodes(node_embeddings, active_tail)
                if use_path_state:
                    path_features = gather_nodes(
                        state.path_state_features(node_embeddings), active_tail
                    )
                ...
                selected_tail_pos = _select(tail_log_p, decode_type, generator)
                selected_tail = active_tail.gather(1, selected_tail_pos[:, None]).squeeze(1)

            if torch.is_grad_enabled():
                selected_query = active_tail_queries.gather(
                    1,
                    selected_tail_pos[:, None, None].expand(
                        state.batch_size, 1, self.embedding_dim
                    ),
                ).squeeze(1)
                selected_mask = pair_mask[batch_index, selected_tail_pos]
                selected_head_logits = self._attention_logits(
                    selected_query,
                    active_head_key,
                    active_head_value,
                    active_head_logit_key,
                    selected_mask,
                    self.project_head_glimpse,
                )
                ...
                selected_head_log_p = all_head_log_p[batch_index, selected_tail_pos]

            selected_head_pos = _select(selected_head_log_p, decode_type, generator)
            selected_head = active_head.gather(
                1, selected_head_pos[:, None]
            ).squeeze(1)
            if pair_mask[batch_index, selected_tail_pos, selected_head_pos].any():
                raise RuntimeError("Forest-aware decoder selected an illegal edge")

            selected_log_probabilities.append(
                tail_log_p.gather(1, selected_tail_pos[:, None]).squeeze(1)
                + selected_head_log_p.gather(1, selected_head_pos[:, None]).squeeze(1)
            )
            with torch.no_grad():
