        ...
        *,
        ...
        while not process.is_terminal(state):
            ...
            if random_tail:
                ...
                # Bind each construction-step forest state to a distinct closure.
                # Checkpoint recomputation happens later during backward; a loop-scoped
                # shared closure cell would otherwise observe the final rollout state.
                def make_tail_policy(state_snapshot, tail_mask_snapshot):
                    def tail_policy(
                        ...
                        if use_path_state:
                            state_tail_nodes = state_tail_nodes + self.project_tail_state(
                                state_snapshot.path_state_features(node_embeddings_in)
                            )
                        if use_head_summary:
                ...
                tail_policy = make_tail_policy(state, tail_mask)
