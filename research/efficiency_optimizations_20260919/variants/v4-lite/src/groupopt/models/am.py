...
class AttentionModel(nn.Module):
    ...
    def _decode_native_conditional_free(
        ...
        *,
        ...
        """Efficient dense Forest-aware decoding.

        Full ASCC keeps the canonical all-source head-proposal lookahead. When
        use_head_summary is false (the ASCC-Lite path), the tail policy depends
        only on forest/path state, so no all-tail head proposal or NxN action mask is
        constructed. After selecting a tail, only that tail's head distribution is
        scored. In no-grad evaluation, entropy diagnostics are skipped because they
        do not affect the constructed solution.
        """
        process = self.construction_process
        state = process.initial_state(coordinates)
        graph_context = self.project_graph(graph_embedding)
        ...
        proposal_needed = use_head_summary and not fixed_tail and not random_tail
        distances = (
            torch.cdist(coordinates[..., :2], coordinates[..., :2])
            if proposal_needed
            else None
        )
        collect_metrics = torch.is_grad_enabled()
        ...
        while not process.is_terminal(state):
            ...
            if torch.is_grad_enabled() or all_head_log_p is None:
            ...
            if selected_mask[batch_index, selected_head].any():
            ...
            if collect_metrics:
                with torch.no_grad():
                    ...
                    if all_head_log_p is not None:
                        ...
                        action_entropy = tail_entropy + (
                            tail_probabilities * head_entropy
                        ).sum(dim=-1)
                    else:
                        action_entropy = tail_entropy + categorical_entropy(
                            selected_head_log_p.detach()
                        )
        ...
        return AttentionModelOutput(
            ...
            successor=process.solution(state),
            tail_entropy=(
                torch.stack(tail_entropies, dim=1).mean(dim=1)
                if tail_entropies else zeros
            ),
            action_entropy=(
                torch.stack(action_entropies, dim=1).mean(dim=1)
                if action_entropies else zeros
            ),
        )
